    ...
    async fetchData(phoneId) {
        try {
            ...
            $('.panel').each((_, el) => {
                ...
                // Regen – erweitert
                let rainTotal = text.match(/Gesamt\s+([\d,.-]+)\s*mm/i);
                if (!rainTotal) rainTotal = text.match(/Regen(?:menge)?\s+([\d,.-]+)\s*mm/i);
                ...
                // Wind
                const wind = text.match(/Wind\s+([\d,.-]+)\s*m\/s/i);
                if (wind) data.wind = this.convertWind(num(wind[1]));

                // Batterie
                const bat = text.match(/Batterie\s+(\w+)/i);
                if (bat) data.battery = bat[1] === 'OK' ? 1 : 0;

                // Zeitstempel
                const ts = text.match(/Zeitpunkt\s+([\d.]+\s+[\d:]+)/i);
                if (ts) data.timestamp = ts[1];
            ...
            for (const sensor of sensors) {
                ...
                for (const key in sensor.data) {
                    ...
                    await this.setStateAsync(`${sensorBase}.${key}`, { val: sensor.data[key], ack: true });
                }

                // -------------------- Regen-Berechnungen --------------------
                if (sensor.data.rain_total !== undefined) {

                    const cur = sensor.data.rain_total;
                    ...
                    const weekday = now.getDay(); // 1 = Montag
                    ...
                    // neue Stunde
                    if (!hourRef || hour !== new Date(hourRef.ts).getHours()) {
                        await this.setStateAsync(hKey, { val: cur, ack: true });
                    }

                    // neuer Tag
                    if (!dayRef || day !== new Date(dayRef.ts).getDate()) {
                        await this.setStateAsync(dKey, { val: cur, ack: true });
                    }

                    // neue Woche (Montag 00:00)
                    if (!weekRef || (weekday === 1 && now.getHours() === 0)) {
                        await this.setStateAsync(wKey, { val: cur, ack: true });
                    }

                    // neuer Monat
                    if (!monthRef || (month !== (new Date(monthRef.ts).getMonth() + 1))) {
                    ...
                    for (const k in extra) {
                        ...
                        await this.setStateAsync(`${sensorBase}.${k}`, { val: extra[k], ack: true });
                    }
                }
                // ------------------------------------------------------------------
            }
